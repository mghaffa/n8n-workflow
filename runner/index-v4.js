...
// GPT: strict JSON schema + tolerant parsing (from your working file)
// Grok: same as your v3 with /chat and /responses fallback
...
// Requested models
const OPENAI_MODEL   = envTrim("OPENAI_MODEL", "gpt-4o");             // use gpt-4o as requested
const XAI_MODEL      = envTrim("XAI_MODEL", "grok-4-fast-reasoning"); // override via Secrets if needed
...
/* ---------- tolerant JSON parsing helpers (from your working file) ---------- */
...
async function callGrok(prompt) {
  ...
  if (r.status === 404) {
    // Fallback to /responses if chat is unavailable
    const fb = {
      model: XAI_MODEL,
      input: [
        { role: "system", content: "You are an equity options screener. Use ONLY the text inside PROMPT CORPUS. Return STRICT JSON {results:[{ticker, sentiment, catalysts, rationale, suggested_spread, confidence}]}." },
...
async function main() {
  ...
  // combine with sensible fallbacks (same logic you liked)
  let grokBanner = "";
  let combined;
  ...
  if (!DRY_RUN && (!Array.isArray(gpt.results) || gpt.results.length === 0) && (!Array.isArray(grok.results) || grok.results.length === 0)) {
    ...
    grokBanner = "OpenAI returned no structured results; using Grok + news heuristics.";
