// runner/index.js
// Bullet Catalyst (GitHub runner) — OpenAI + xAI (Grok) screen with clear fallbacks.
...
const GROK_HEURISTIC_FALLBACK = asBool(process.env.GROK_HEURISTIC_FALLBACK, true); // <- toggle

console.log("[env] OPENAI_API_KEY:", mask(OPENAI_API_KEY));
console.log("[env] XAI_API_KEY   :", mask(XAI_API_KEY));
console.log("[env] GROK_HEURISTIC_FALLBACK:", GROK_HEURISTIC_FALLBACK);
...
const TICKER_BLACKLIST = new Set(["IN","WITH","AND","THE","FOR","FROM","OVER","AFTER","BEFORE","FIRST","SECOND","THIRD","NEWS","CNBC","CNN","TECH","STOCK","MARKET","EARNINGS","RESULTS","SHARES","OFF","S","INTEL"]);
...
async function callOpenAI(prompt) {
  ...
  const body = {
    model: OPENAI_MODEL,
    response_format: { type: "json_object" }, // safer than json_schema across models
    messages: [
  ...
  try { const json = JSON.parse(content); json._ok = true; return json; }
  catch(e){ console.error("[openai] JSON parse failed"); return { results: [], _err:"parse" }; }
}
...
async function callGrok(prompt) {
  ...
  const content = r?.data?.choices?.[0]?.message?.content || "{}";
  try { const json = JSON.parse(content); json._ok = true; return json; }
  catch(e){ console.error("[grok] JSON parse failed"); return { results: [], _err:"parse" }; }
}
...
function normalizeResults(baseTickers, gpt, grok) {
  ...
  for (const T of baseTickers) {
    ...
    const sGROK = Number.isFinite(Number(b.sentiment)) ? Number(b.sentiment) : 48; // neutral-ish
    const cats  = uniqueCaseFold([...(a.catalysts || []), ...(b.catalysts || [])]);
    const bonus = 0.1 * scoreCatalysts(cats);
    out.push({
      ticker:T,
      sentiment_gpt:sGPT, sentiment_grok:sGROK,
...
// ---------------- 5) Markdown ----------------
function toMarkdown(topGpt, topGrok, newsByTicker, grokBanner) {
  ...
  const grokHeader = grokBanner ? `> ${grokBanner}\n\n` : "";
  return `# Daily Top 10 — Call-Spread Screen (News-driven)

${grokHeader}## Top 10 — GPT
...
async function main() {
  ...
  let grokBanner = "";
  let effectiveGrok = grok;

  if (!Array.isArray(grok.results) || grok.results.length === 0) {
    // Grok unavailable → fall back (optional)
    if (GROK_HEURISTIC_FALLBACK) {
      ...
      grokBanner = `**${reason}; using OpenAI + news heuristics for the Grok section.**`;
      effectiveGrok = { results: [] }; // heuristics are added via merge below
    } else {
      grokBanner = "**Grok disabled (no API results).**";
      effectiveGrok = { results: [] };
    }
  }

  // If either provider returned absolutely nothing, still allow the email to go out based on news-derived tickers
  const combined = normalizeResults(baseTickers, gpt || {results:[]}, effectiveGrok || {results:[]});
  ...
  const md = toMarkdown(topGpt, topGrok, byTicker, grokBanner);
