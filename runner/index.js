...
// runner/index.js — debugged version
...
function extractTickers(text, url) {
  ...
  const out = [];
  for (const sym of set) { const S = String(sym).toUpperCase().trim(); if (!/^[A-Z]{1,5}$/.test(S)) continue; if (TICKER_BLACKLIST.has(S)) continue; out.push(S); }
...
async function callGrok(prompt) {
  ...
  const body = {
    model: XAI_MODEL,
    // If your account/model doesn’t support response_format, remove the next line:
...
async function main() {
  ...
  // If either model returned nothing, fail loudly so you notice (instead of silently showing 50/50s)
  if (!DRY_RUN && (!Array.isArray(gpt.results) || !Array.isArray(grok.results) || gpt.results.length === 0 || grok.results.length === 0)) {
    throw new Error("Model returned no results (see [openai]/[grok] status logs above). Check API keys, quotas, or model names.");
  }
