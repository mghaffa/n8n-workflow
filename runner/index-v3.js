...
const XAI_MODEL      = envTrim("XAI_MODEL", "grok-4-fast-reasoning"); // override in Secrets if needed
...
const GROK_HEURISTIC_FALLBACK = asBool(process.env.GROK_HEURISTIC_FALLBACK, true);
...
async function callGrok(prompt) {
  ...
  if (r.status === 404) {
    // Fallback to /responses if chat is unavailable for your key
    const fb = {
      model: XAI_MODEL,
      input: [
        { role: "system", content: "You are an equity options screener. Use ONLY the text inside PROMPT CORPUS. Return STRICT JSON {results:[{ticker, sentiment, catalysts, rationale, suggested_spread, confidence}]}." },
...
function normalizeResults(baseTickers, gpt, grok) {
  ...
  for (const T of baseTickers) {
    ...
    const sGROK = Number.isFinite(Number(b.sentiment)) ? Number(b.sentiment) : 48; // neutral-ish when missing
